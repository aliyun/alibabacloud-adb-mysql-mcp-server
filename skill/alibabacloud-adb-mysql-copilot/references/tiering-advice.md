...
## 1. 功能
用于检测 ADB MySQL 实例是否存在冷热分层存储优化建议。

## 2. 调用方式
```bash
# 默认地域 cn-hangzhou
uv run ./scripts/call_adb_api.py describe_available_advices --cluster-id amv-xx --advice-type TIERING --page-size 5

# 指定地域
uv run ./scripts/call_adb_api.py describe_available_advices --region cn-shanghai --cluster-id amv-xx --advice-type TIERING --page-size 5
```
...
| `advice-type` | string | 是 | 固定传 "TIERING" |
...
## 7. 展示模板
### 诊断报告：冷热表优化建议
- **风险说明**：最近15天未访问且访问率小于1%的热表，导致整体成本过高。
- **优化建议**：开启冷热分层存储。前往【空间诊断 > 表诊断 > 冷热表优化】，将低频访问的热表转换为冷表。

| 数据库.表名 | SQL | 具体优化建议 | 预期优化收益 |
| :--- | :--- | :--- | :--- |
| {SchemaName}.{TableName} | {SQL} | {Reason} | {Benefit} |